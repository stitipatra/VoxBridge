...
def translate_segments(
    ...
    log_timing("========== TRANSLATION DIAGNOSTICS ==========")
    log_timing(f"Translation groups: {len(grouped_segments)}")
    ...
    if grouped_segments:
        ...
        log_timing(
            f"Character lengths:"
            f" min={min(group_lengths)},"
            f" max={max(group_lengths)},"
            f" avg={sum(group_lengths)/len(group_lengths):.2f}"
        )

        log_timing(
            f"Group lengths: {group_lengths}"
        )
    ...
    if grouped_segments:
        ...
        print(
            "Translation group character lengths: "
            f"min={min(group_lengths)}, "
            f"max={max(group_lengths)}, "
            f"average={sum(group_lengths) / len(group_lengths):.2f}",
            flush=True
        )

        print(
            f"All translation group lengths: {group_lengths}",
            flush=True
        )
